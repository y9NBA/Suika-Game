...
class Particle:
    ...
    def draw(self, screen):
        if self.alive:
            ...
            pg.draw.circle(screen, tuple(c2), self.body.position, self.radius)
            pg.draw.circle(screen, tuple(c1), self.body.position, self.radius * 0.9)
...
class PreParticle:
    ...
    def draw(self, screen):
        c1 = np.array(COLORS[self.n])
        c2 = (c1 * 0.8).astype(int)
        pg.draw.circle(screen, tuple(c2), (self.x, PAD[1] // 2), self.radius)
        pg.draw.circle(screen, tuple(c1), (self.x, PAD[1] // 2), self.radius * 0.9)
...
class Wall:
    ...
    def draw(self, screen):
        pg.draw.line(screen, W_COLOR, self.shape.a, self.shape.b, self.thickness)
